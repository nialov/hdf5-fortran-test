...
!   integer :: error
!   integer, dimension(9) :: time_data

!   call h5open_f(error)
!   if (error /= 0) stop "Error opening HDF5"

!   ! call h5gmtime(time_data, error)
!   ! if (error /= 0) stop "Error getting time"

!   ! print *, "Current time: ", time_data

!   call h5close_f(error)
!   if (error /= 0) stop "Error closing HDF5"
